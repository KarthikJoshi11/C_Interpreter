...
class Interpreter(NodeVisitor):
    ...
    def visit_VarDecl(self, node):
        if node.var_node.array_size:
            # For array declarations, initialize with list of zeros
            size = self.visit(node.var_node.array_size)
            try:
                array = [Number('int', 0)] * size.value
                self.memory.declare(node.var_node.value, array)
            except (TypeError, ValueError, AttributeError):
                # Handle case where size.value doesn't exist
                self.memory.declare(node.var_node.value, [Number('int', 0)])
        else:
    ...
    def visit_Var(self, node):
        if node.array_size:
            # For array access, return tuple of (array_name, index)
            index = self.visit(node.array_size)
            try:
                value = self.memory[(node.value, index.value)]
                ...
                # Handle case where index.value doesn't exist
                return Number('int', 0)
        return self.memory[node.value]

    def visit_Assign(self, node):
        if isinstance(node.left, Var) and node.left.array_size:
            # For array assignment
            index = self.visit(node.left.array_size)
            value = self.visit(node.right)
            try:
                if isinstance(value, str):
                    self.memory[(node.left.value, index.value)] = value
                elif isinstance(value, (int, float)):
                    self.memory[(node.left.value, index.value)] = Number('int', value)
                elif value is None:
                    self.memory[(node.left.value, index.value)] = Number('int', 0)
                else:
                    self.memory[(node.left.value, index.value)] = value
                return value
            except (KeyError, TypeError, AttributeError):
